...
describe('A BasePasswordStore', (): void => {
    ...
    it('defines the type and indexes in the storage.', async (): Promise<void> => {
        ...
        expect(storage.defineType).toHaveBeenLastCalledWith(
            STORAGE_TYPE,
            {
                email: 'string',
                password: 'string',
    ...
    it('can create logins.', async (): Promise<void> => {
        ...
        expect(storage.create).toHaveBeenLastCalledWith(STORAGE_TYPE, { accountId, verified: false, email: lowercase, password: expect.any(String) });
    });
