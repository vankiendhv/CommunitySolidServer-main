...
import { CLIENT_CREDENTIALS_STORAGE_DESCRIPTION, CLIENT_CREDENTIALS_STORAGE_TYPE } from '../../identity/interaction/client-credentials/util/BaseClientCredentialsStore';
import { PASSWORD_STORAGE_DESCRIPTION, PASSWORD_STORAGE_TYPE } from '../../identity/interaction/password/util/BasePasswordStore';
import { OWNER_STORAGE_DESCRIPTION, OWNER_STORAGE_TYPE, POD_STORAGE_DESCRIPTION, POD_STORAGE_TYPE } from '../../identity/interaction/pod/util/BasePodStore';
...
export class V6MigrationInitializer extends Initializer {
     ...
     */
    protected async createAccount(account: Account | Settings): Promise<{ accountId: string; webId: string } | undefined> {
        if (!this.isAccount(account)) {
            return;
        }

        const { webId, email, password, verified, username } = account;
        ...
        await this.newAccountStorage.create(PASSWORD_STORAGE_TYPE, { email: email.toLowerCase(), password, verified, accountId, username });
        if (settings.useIdp) {
            await this.newAccountStorage.create(WEBID_STORAGE_TYPE, { webId, accountId });
        }
        if (settings.podBaseUrl) {
            const { id: podId } = await this.newAccountStorage.create(POD_STORAGE_TYPE, { baseUrl: settings.podBaseUrl, accountId });
