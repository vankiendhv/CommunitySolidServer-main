...
export class BasePasswordStore extends Initializer implements PasswordStore {
    ...
    public async handle(): Promise<void> {
        ...
        try {
            ...
            throw new InternalServerError(`Error defining email/password in storage: ${createErrorMessage(cause)}`, { cause });
        }
    }

    public async create(email: string, accountId: string, password: string, username?: string): Promise<string> {
        console.log('BasePasswordStore  username:', username);
        console.log('BasePasswordStore  password:', password);
        console.log('BasePasswordStore  accountId:', accountId);
        console.log('BasePasswordStore  email:', email);
    ...
    public async findByAccount(accountId: string): Promise<{ id: string; email: string }[]> {
        return (await this.storage.find(PASSWORD_STORAGE_TYPE, { accountId })).map(({ id, email }): { id: string; email: string } => ({ id, email }));
    }
